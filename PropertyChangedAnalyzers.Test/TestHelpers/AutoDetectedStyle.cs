    ...
    public readonly struct AutoDetectedStyle
    {
        ...
        public string Apply(string source, params string[] namesToStyle)
        {
            foreach (var name in namesToStyle)
            {
                ...
                {
                    styledName = this.applyFieldNamingStyle(name);

                    source = source.AssertReplaceWholeWord(name, styledName);
                }
                else
                {
                    // May not be a field, e.g. PropertyChanged
                    styledName = name;
                }

                source = source.ReplaceWholeWord("this." + styledName, this.applyFieldQualificationPreference(styledName));
            }
